...
const {
    addMessage, addMessages, parseMessage, getRememberedMessages, clearRememberedMessages, rememberRangeGrab
...
const startRemember = async(client, interaction)=>
{
    ...
    {
        await interaction.editReply({ content: `Already remembering in ${rememberMessageObj.id}. Use "/stop-remembering" to stop remembering.` });
        return;
    }
    ...
    const obj = {
        id: channel.id, last_message_id: last_message_id, name: name, excludeBotMessages: excludeBotMessages, ephemeral: ephemeral.startRemember
    };
    rememberMessageObj = obj;
    await interaction.editReply({ content: `Starting to remember messages in <#${channel}>."` });
...
const stopRemember = async(client, interaction)=>
{
    ...
    if (!obj)
    {
        await interaction.editReply({ content: `No channel is being remembered. Use "start-remembering" to start remember messages in a channel` });
        return;
    }
